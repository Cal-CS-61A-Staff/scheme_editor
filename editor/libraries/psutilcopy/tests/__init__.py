...
from psutilcopy._compat import unicode
from psutilcopy._compat import which
...
HAS_CPU_AFFINITY = hasattr(psutilcopy.Process, "cpu_affinity")
HAS_CPU_FREQ = hasattr(psutilcopy, "cpu_freq")
HAS_CONNECTIONS_UNIX = POSIX and not SUNOS
HAS_ENVIRON = hasattr(psutilcopy.Process, "environ")
HAS_PROC_IO_COUNTERS = hasattr(psutilcopy.Process, "io_counters")
HAS_IONICE = hasattr(psutilcopy.Process, "ionice")
HAS_MEMORY_FULL_INFO = 'uss' in psutilcopy.Process().memory_full_info()._fields
HAS_MEMORY_MAPS = hasattr(psutilcopy.Process, "memory_maps")
HAS_PROC_CPU_NUM = hasattr(psutilcopy.Process, "cpu_num")
HAS_RLIMIT = hasattr(psutilcopy.Process, "rlimit")
HAS_THREADS = hasattr(psutilcopy.Process, "threads")
HAS_SENSORS_BATTERY = hasattr(psutilcopy, "sensors_battery")
HAS_BATTERY = HAS_SENSORS_BATTERY and bool(psutilcopy.sensors_battery())
HAS_SENSORS_FANS = hasattr(psutilcopy, "sensors_fans")
HAS_SENSORS_TEMPERATURES = hasattr(psutilcopy, "sensors_temperatures")
...
def _get_py_exe():
    ...
    if OSX:
        exe = \
            ...
            attempt(psutilcopy.Process().exe())
        if not exe:
...
VALID_PROC_STATUSES = [getattr(psutilcopy, x) for x in dir(psutilcopy)
...
def reap_children(recursive=False):
    ...
    #     jiq2cgd6stsbtn60
    def assert_gone(pid):
        assert not psutilcopy.pid_exists(pid), pid
        assert pid not in psutilcopy.pids(), pid
        try:
    ...
    if recursive:
        children = set(psutilcopy.Process().children(recursive=True))
    else:
        children = set()
    ...
    if children:
        for p in children:
            try:
                ...
                pass
        gone, alive = psutilcopy.wait_procs(children, timeout=GLOBAL_TIMEOUT)
        for p in alive:
            ...
            try:
                ...
                pass
        gone, alive = psutilcopy.wait_procs(alive, timeout=GLOBAL_TIMEOUT)
        if alive:
            for p in alive:
...
@retry(exception=psutilcopy.NoSuchProcess, logfun=None, timeout=GLOBAL_TIMEOUT,
       interval=0.001)
def wait_for_pid(pid):
    """Wait for pid to show up in the process list then return.
    Used in the test suite to give time the sub process to initialize.
    """
    psutilcopy.Process(pid)
    if WINDOWS:
...
def _setup_tests():
    ...
    psutilcopy._psplatform.cext.set_testing()
...
def check_net_address(addr, family):
    ...
    elif family == psutilcopy.AF_LINK:
...
def check_connection_ntuple(conn):
    ...
    if conn.family in (AF_INET, AF_INET6):
        ...
        assert conn.status == psutilcopy.CONN_NONE, conn.status
    ...
    valids = [getattr(psutilcopy, x) for x in dir(psutilcopy) if x.startswith('CONN_')]
...
if POSIX:
    ...
    def copyload_shared_lib(dst_prefix=TESTFILE_PREFIX):
        ...
        libs = [x.path for x in psutilcopy.Process().memory_maps() if
