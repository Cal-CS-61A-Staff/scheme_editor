...
class TestProcess(unittest.TestCase):
    ...
    def test_ppid(self):
        ...
        ppid_psutil = psutilcopy.Process(self.pid).ppid()
        self.assertEqual(ppid_ps, ppid_psutil)

    def test_uid(self):
        uid_ps = ps("ps --no-headers -o uid -p %s" % self.pid)
        uid_psutil = psutilcopy.Process(self.pid).uids().real
        self.assertEqual(uid_ps, uid_psutil)

    def test_gid(self):
        gid_ps = ps("ps --no-headers -o rgid -p %s" % self.pid)
        gid_psutil = psutilcopy.Process(self.pid).gids().real
        self.assertEqual(gid_ps, gid_psutil)

    def test_username(self):
        username_ps = ps("ps --no-headers -o user -p %s" % self.pid)
        username_psutil = psutilcopy.Process(self.pid).username()
    ...
    def test_rss_memory(self):
        ...
        rss_psutil = psutilcopy.Process(self.pid).memory_info()[0] / 1024
    ...
    def test_vsz_memory(self):
        ...
        vsz_psutil = psutilcopy.Process(self.pid).memory_info()[1] / 1024
    ...
    def test_name(self):
        ...
        name_psutil = psutilcopy.Process(self.pid).name().lower()
    ...
    def test_name_long_cmdline_ad_exc(self):
        ...
        with mock.patch("psutil._psplatform.Process.name",
                        return_value=name):
            with mock.patch("psutil._psplatform.Process.cmdline",
                            side_effect=psutilcopy.AccessDenied(0, "")):
                p = psutilcopy.Process()
    ...
    def test_name_long_cmdline_nsp_exc(self):
        ...
        with mock.patch("psutil._psplatform.Process.name",
                        return_value=name):
            with mock.patch("psutil._psplatform.Process.cmdline",
                            side_effect=psutilcopy.NoSuchProcess(0, "")):
                p = psutilcopy.Process()
                self.assertRaises(psutilcopy.NoSuchProcess, p.name)

    @unittest.skipIf(OSX or BSD, 'ps -o start not available')
    def test_create_time(self):
        time_ps = ps("ps --no-headers -o start -p %s" % self.pid).split(' ')[0]
        time_psutil = psutilcopy.Process(self.pid).create_time()
    ...
    def test_exe(self):
        ...
        psutil_pathname = psutilcopy.Process(self.pid).exe()
        try:
    ...
    def test_cmdline(self):
        ...
        psutil_cmdline = " ".join(psutilcopy.Process(self.pid).cmdline())
    ...
    def test_nice(self):
        ...
        psutil_nice = psutilcopy.Process().nice()
...
class TestSystemAPIs(unittest.TestCase):
    ...
    def test_nic_names(self):
        ...
        for nic in psutilcopy.net_io_counters(pernic=True).keys():
            for line in output.split():
    ...
    def test_users(self):
        ...
        self.assertEqual(len(users), len(psutilcopy.users()))
        for u in psutilcopy.users():
    ...
    def test_pid_exists_let_raise(self):
        ...
        with mock.patch("psutil._psposix.os.kill",
            ...
            self.assertRaises(OSError, psutilcopy._psposix.pid_exists, os.getpid())
            assert m.called
    ...
    def test_os_waitpid_let_raise(self):
        ...
        with mock.patch("psutil._psposix.os.waitpid",
                        side_effect=OSError(errno.EBADF, "")) as m:
            self.assertRaises(OSError, psutilcopy._psposix.wait_pid, os.getpid())
            assert m.called
    ...
    def test_os_waitpid_eintr(self):
        ...
        with mock.patch("psutil._psposix.os.waitpid",
            ...
            self.assertRaises(
                psutilcopy._psposix.TimeoutExpired,
                psutilcopy._psposix.wait_pid, os.getpid(), timeout=0.01)
            assert m.called
    ...
    def test_os_waitpid_bad_ret_status(self):
        ...
        with mock.patch("psutil._psposix.os.waitpid",
            ...
            self.assertRaises(ValueError,
                              psutilcopy._psposix.wait_pid, os.getpid())
            assert m.called
