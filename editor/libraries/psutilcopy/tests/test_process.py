...
from psutilcopy.tests import copyload_shared_lib
from psutilcopy.tests import create_exe
from psutilcopy.tests import create_proc_children_pair
...
from psutilcopy.tests import get_winver
from psutilcopy.tests import HAS_CPU_AFFINITY
from psutilcopy.tests import HAS_ENVIRON
from psutilcopy.tests import HAS_IONICE
from psutilcopy.tests import HAS_MEMORY_MAPS
from psutilcopy.tests import HAS_PROC_CPU_NUM
from psutilcopy.tests import HAS_PROC_IO_COUNTERS
from psutilcopy.tests import HAS_RLIMIT
from psutilcopy.tests import HAS_THREADS
from psutilcopy.tests import mock
from psutilcopy.tests import PYPY
from psutilcopy.tests import PYTHON_EXE
...
from psutilcopy.tests import skip_on_access_denied
from psutilcopy.tests import skip_on_not_implemented
from psutilcopy.tests import TESTFILE_PREFIX
...
from psutilcopy.tests import wait_for_pid
from psutilcopy.tests import WIN_VISTA
...
class TestProcess(unittest.TestCase):
    ...
    def test_pid(self):
        ...
        self.assertEqual(psutilcopy.Process(sproc.pid).pid, sproc.pid)
    ...
    def test_kill(self):
        sproc = get_test_subprocess()
        test_pid = sproc.pid
        p = psutilcopy.Process(test_pid)
        p.kill()
        sig = p.wait()
        self.assertFalse(psutilcopy.pid_exists(test_pid))
        if POSIX:
    ...
    def test_send_signal(self):
        ...
        p.send_signal(sig)
        exit_sig = p.wait()
        self.assertFalse(psutilcopy.pid_exists(p.pid))
        if POSIX:
            ...
            with mock.patch('psutil.os.kill',
                ...
                with self.assertRaises(psutilcopy.NoSuchProcess):
                    p.send_signal(sig)
            #
            sproc = get_test_subprocess()
            p = psutilcopy.Process(sproc.pid)
            ...
            with mock.patch('psutil.os.kill',
                ...
                with self.assertRaises(psutilcopy.AccessDenied):
                    psutilcopy.Process().send_signal(sig)
            ...
            if 0 in psutilcopy.pids():
                p = psutilcopy.Process(0)
    ...
    def test_wait(self):
        ...
        p = psutilcopy.Process(sproc.pid)
        p.name()
        self.assertRaises(psutilcopy.TimeoutExpired, p.wait, 0.01)
    ...
    def test_wait_non_children(self):
        ...
        self.assertRaises(psutilcopy.TimeoutExpired, p1.wait, 0.01)
        self.assertRaises(psutilcopy.TimeoutExpired, p2.wait, 0.01)
    ...
    def test_wait_timeout_0(self):
        sproc = get_test_subprocess()
        p = psutilcopy.Process(sproc.pid)
        self.assertRaises(psutilcopy.TimeoutExpired, p.wait, 0)
        p.kill()
        stop_at = time.time() + 2
        while True:
            try:
                code = p.wait(0)
            except psutilcopy.TimeoutExpired:
    ...
    def test_cpu_percent_numcpus_none(self):
        ...
        with mock.patch('psutil.cpu_count', return_value=None) as m:
            psutilcopy.Process().cpu_percent()
            assert m.called

    def test_cpu_times(self):
        times = psutilcopy.Process().cpu_times()
    ...
    def test_cpu_times_2(self):
        user_time, kernel_time = psutilcopy.Process().cpu_times()[:2]
    ...
    def test_cpu_num(self):
        ...
        if psutilcopy.cpu_count() == 1:
            self.assertEqual(num, 0)
        self.assertIn(p.cpu_num(), range(psutilcopy.cpu_count()))
    ...
    def test_terminal(self):
        terminal = psutilcopy.Process().terminal()
    ...
    def test_ionice(self):
        if LINUX:
            from psutilcopy import (IOPRIO_CLASS_NONE, IOPRIO_CLASS_RT,
    ...
    def test_ionice_errs(self):
        ...
        if LINUX:
            ...
            self.assertRaisesRegex(
                ...
                p.ionice, psutilcopy.IOPRIO_CLASS_NONE, 1)
            self.assertRaisesRegex(
                ValueError, "can't specify value with IOPRIO_CLASS_IDLE",
                p.ionice, psutilcopy.IOPRIO_CLASS_IDLE, 1)
            self.assertRaisesRegex(
    ...
    def test_rlimit_get(self):
        import resource
        p = psutilcopy.Process(os.getpid())
        names = [x for x in dir(psutilcopy) if x.startswith('RLIMIT')]
        assert names, names
        for name in names:
            value = getattr(psutilcopy, name)
    ...
    def test_rlimit_set(self):
        sproc = get_test_subprocess()
        p = psutilcopy.Process(sproc.pid)
        p.rlimit(psutilcopy.RLIMIT_NOFILE, (5, 5))
        self.assertEqual(p.rlimit(psutilcopy.RLIMIT_NOFILE), (5, 5))
        # If pid is 0 prlimit() applies to the calling process and
        # we don't want that.
        with self.assertRaises(ValueError):
            psutilcopy._psplatform.Process(0).rlimit(0)
        with self.assertRaises(ValueError):
            p.rlimit(psutilcopy.RLIMIT_NOFILE, (5, 5, 5))

    @unittest.skipIf(not HAS_RLIMIT, "not supported")
    def test_rlimit(self):
        p = psutilcopy.Process()
        soft, hard = p.rlimit(psutilcopy.RLIMIT_FSIZE)
        try:
            p.rlimit(psutilcopy.RLIMIT_FSIZE, (1024, hard))
            ...
            p.rlimit(psutilcopy.RLIMIT_FSIZE, (soft, hard))
            self.assertEqual(p.rlimit(psutilcopy.RLIMIT_FSIZE), (soft, hard))
    ...
    def test_rlimit_infinity(self):
        ...
        try:
            p.rlimit(psutilcopy.RLIMIT_FSIZE, (1024, hard))
            p.rlimit(psutilcopy.RLIMIT_FSIZE, (psutilcopy.RLIM_INFINITY, hard))
            with open(TESTFN, "wb") as f:
                f.write(b"X" * 2048)
        finally:
            p.rlimit(psutilcopy.RLIMIT_FSIZE, (soft, hard))
    ...
    def test_rlimit_infinity_value(self):
        ...
        p = psutilcopy.Process()
        soft, hard = p.rlimit(psutilcopy.RLIMIT_FSIZE)
        self.assertEqual(psutilcopy.RLIM_INFINITY, hard)
        p.rlimit(psutilcopy.RLIMIT_FSIZE, (soft, hard))
    ...
    def test_num_threads(self):
        ...
        if OPENBSD:
            try:
                step1 = p.num_threads()
            except psutilcopy.AccessDenied:
    ...
    def test_memory_full_info(self):
        total = psutilcopy.virtual_memory().total
        mem = psutilcopy.Process().memory_full_info()
        for name in mem._fields:
    ...
    def test_memory_maps_lists_lib(self):
        ...
        with copyload_shared_lib() as path:
            ...
            libpaths = [normpath(x.path)
                        for x in psutilcopy.Process().memory_maps()]
    ...
    def test_exe(self):
        sproc = get_test_subprocess()
        exe = psutilcopy.Process(sproc.pid).exe()
        try:
    ...
    def test_cmdline(self):
        ...
        try:
            self.assertEqual(' '.join(psutilcopy.Process(sproc.pid).cmdline()),
                             ' '.join(cmdline))
        except AssertionError:
            ...
            if NETBSD or OPENBSD or AIX:
                self.assertEqual(
                    psutilcopy.Process(sproc.pid).cmdline()[0], PYTHON_EXE)
            else:
                raise

    def test_name(self):
        sproc = get_test_subprocess(PYTHON_EXE)
        name = psutilcopy.Process(sproc.pid).name().lower()
    ...
    def test_nice(self):
        ...
        if WINDOWS:
            try:
                ...
                self.assertEqual(init, psutilcopy.NORMAL_PRIORITY_CLASS)
                p.nice(psutilcopy.HIGH_PRIORITY_CLASS)
                self.assertEqual(p.nice(), psutilcopy.HIGH_PRIORITY_CLASS)
                p.nice(psutilcopy.NORMAL_PRIORITY_CLASS)
                self.assertEqual(p.nice(), psutilcopy.NORMAL_PRIORITY_CLASS)
            finally:
                p.nice(psutilcopy.NORMAL_PRIORITY_CLASS)
        else:
            first_nice = p.nice()
    ...
    def test_status(self):
        p = psutilcopy.Process()
        self.assertEqual(p.status(), psutilcopy.STATUS_RUNNING)
    ...
    def test_cpu_affinity(self):
        ...
        all_cpus = list(range(len(psutilcopy.cpu_percent(percpu=True))))
    ...
    def test_cpu_affinity_errs(self):
        sproc = get_test_subprocess()
        p = psutilcopy.Process(sproc.pid)
        invalid_cpu = [len(psutilcopy.cpu_times(percpu=True)) + 10]
    ...
    def test_ppid(self):
        if hasattr(os, 'getppid'):
            self.assertEqual(psutilcopy.Process().ppid(), os.getppid())
    ...
    def test_parent_disappeared(self):
        ...
        with mock.patch("psutil.Process",
                        side_effect=psutilcopy.NoSuchProcess(0, 'foo')):
    ...
    def test_children_duplicates(self):
        ...
        for p in psutilcopy.process_iter():
            try:
                table[p.ppid()] += 1
            except psutilcopy.Error:
                pass
        # this is the one, now let's make sure there are no duplicates
        pid = sorted(table.items(), key=lambda x: x[1])[-1][0]
        p = psutilcopy.Process(pid)
        try:
            c = p.children(recursive=True)
        except psutilcopy.AccessDenied:  # windows
            pass
        else:
    ...
    def test_suspend_resume(self):
        ...
        p.suspend()
        for x in range(100):
            if p.status() == psutilcopy.STATUS_STOPPED:
                break
            time.sleep(0.01)
        p.resume()
        self.assertNotEqual(p.status(), psutilcopy.STATUS_STOPPED)

    def test_invalid_pid(self):
        self.assertRaises(TypeError, psutilcopy.Process, "1")
        self.assertRaises(ValueError, psutilcopy.Process, -1)
    ...
    def test_as_dict(self):
        ...
        p = psutilcopy.Process(min(psutilcopy.pids()))
        ...
        with mock.patch('psutil.Process.nice', create=True,
                        side_effect=psutilcopy.AccessDenied):
            self.assertEqual(
                p.as_dict(attrs=["nice"], ad_value=1), {"nice": 1})

        # Test that NoSuchProcess bubbles up.
        with mock.patch('psutil.Process.nice', create=True,
                        side_effect=psutilcopy.NoSuchProcess(p.pid, "name")):
            self.assertRaises(
                psutilcopy.NoSuchProcess, p.as_dict, attrs=["nice"])

        # Test that ZombieProcess is swallowed.
        with mock.patch('psutil.Process.nice', create=True,
                        side_effect=psutilcopy.ZombieProcess(p.pid, "name")):
            self.assertEqual(
    ...
    def test_halfway_terminated_process(self):
        ...
        p.wait()
        if WINDOWS:
            call_until(psutilcopy.pids, "%s not in ret" % p.pid)
        self.assertFalse(p.is_running())
            ...
            try:
                ...
                if name == 'nice':
                    if POSIX:
                        ...
                        ret = meth(psutilcopy.NORMAL_PRIORITY_CLASS)
                elif name == 'ionice':
                    ret = meth()
                    ret = meth(2)
                elif name == 'rlimit':
                    ret = meth(psutilcopy.RLIMIT_NOFILE)
                    ret = meth(psutilcopy.RLIMIT_NOFILE, (5, 5))
                    ...
                    ret = meth()
            except psutilcopy.ZombieProcess:
                self.fail("ZombieProcess for %r was not supposed to happen" %
                          name)
            except psutilcopy.NoSuchProcess:
                pass
            except psutilcopy.AccessDenied:
    ...
    def test_zombie_process(self):
        def succeed_or_zombie_p_exc(fun, *args, **kwargs):
            ...
            except (psutilcopy.ZombieProcess, psutilcopy.AccessDenied):
                pass

        zpid = create_zombie_proc()
        self.addCleanup(reap_children, recursive=True)
        # A zombie process should always be instantiable
        zproc = psutilcopy.Process(zpid)
        # ...and at least its status always be querable
        self.assertEqual(zproc.status(), psutilcopy.STATUS_ZOMBIE)
        ...
        if hasattr(zproc, "rlimit"):
            succeed_or_zombie_p_exc(zproc.rlimit, psutilcopy.RLIMIT_NOFILE)
            succeed_or_zombie_p_exc(zproc.rlimit, psutilcopy.RLIMIT_NOFILE,
                                    (5, 5))
        # set methods
        ...
        self.assertTrue(psutilcopy.pid_exists(zpid))
        if not TRAVIS and OSX:
            ...
            self.assertIn(zpid, psutilcopy.pids())
            self.assertIn(zpid, [x.pid for x in psutilcopy.process_iter()])
            psutilcopy._pmap = {}
            self.assertIn(zpid, [x.pid for x in psutilcopy.process_iter()])
    ...
    def test_zombie_process_is_running_w_exc(self):
        ...
        p = psutilcopy.Process()
        with mock.patch("psutil.Process",
                        side_effect=psutilcopy.ZombieProcess(0)) as m:
            assert p.is_running()
            assert m.called
    ...
    def test_pid_0(self):
        ...
        if 0 not in psutilcopy.pids():
            self.assertRaises(psutilcopy.NoSuchProcess, psutilcopy.Process, 0)
            return

        # test all methods
        p = psutilcopy.Process(0)
        for name in psutilcopy._as_dict_attrnames:
            if name == 'pid':
                continue
            meth = getattr(p, name)
            try:
        ...
        if hasattr(p, 'rlimit'):
            try:
                p.rlimit(psutilcopy.RLIMIT_FSIZE)
            except psutilcopy.AccessDenied:
                pass
        ...
        if not OPENBSD:
            self.assertIn(0, psutilcopy.pids())
            self.assertTrue(psutilcopy.pid_exists(0))
...
class TestPopen(unittest.TestCase):
    ...
    def test_misc(self):
        ...
        with psutilcopy.Popen(cmd, stdout=subprocess.PIPE,
    ...
    def test_ctx_manager(self):
        with psutilcopy.Popen([PYTHON_EXE, "-V"],
    ...
    def test_kill_terminate(self):
        ...
        with psutilcopy.Popen(cmd, stdout=subprocess.PIPE,
            ...
            proc.terminate()
            proc.wait()
            self.assertRaises(psutilcopy.NoSuchProcess, proc.terminate)
            self.assertRaises(psutilcopy.NoSuchProcess, proc.kill)
            self.assertRaises(psutilcopy.NoSuchProcess, proc.send_signal,
                              signal.SIGTERM)
