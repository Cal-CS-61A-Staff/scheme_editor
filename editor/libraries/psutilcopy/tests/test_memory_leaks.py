...
from psutilcopy._compat import xrange
...
class TestProcessObjectLeaks(TestMemLeak):
    ...
    def test_ionice_set(self):
        if WINDOWS:
            ...
            self.execute(self.proc.ionice, psutilcopy.IOPRIO_CLASS_NONE)
    ...
    def test_rlimit_get(self):
        self.execute(self.proc.rlimit, psutilcopy.RLIMIT_NOFILE)

    @unittest.skipIf(not LINUX, "LINUX only")
    @unittest.skipIf(not HAS_RLIMIT, "not supported")
    def test_rlimit_set(self):
        limit = thisproc.rlimit(psutilcopy.RLIMIT_NOFILE)
        self.execute(self.proc.rlimit, psutilcopy.RLIMIT_NOFILE, limit)
...
class TestTerminatedProcessLeaks(TestProcessObjectLeaks):
    ...
    def setUpClass(cls):
        ...
        cls.proc = psutilcopy.Process(p.pid)
        cls.proc.kill()
        cls.proc.wait()
...
class TestModuleFunctionsLeaks(TestMemLeak):
    ...
    def test_cpu_count_logical(self):
        self.execute(psutilcopy.cpu_count, logical=True)

    @skip_if_linux()
    def test_cpu_count_physical(self):
        self.execute(psutilcopy.cpu_count, logical=False)

    @skip_if_linux()
    def test_cpu_times(self):
        self.execute(psutilcopy.cpu_times)

    @skip_if_linux()
    def test_per_cpu_times(self):
        self.execute(psutilcopy.cpu_times, percpu=True)

    def test_cpu_stats(self):
        self.execute(psutilcopy.cpu_stats)

    @skip_if_linux()
    @unittest.skipIf(not HAS_CPU_FREQ, "not supported")
    def test_cpu_freq(self):
        self.execute(psutilcopy.cpu_freq)

    # --- mem

    def test_virtual_memory(self):
        self.execute(psutilcopy.virtual_memory)

    # TODO: remove this skip when this gets fixed
    @unittest.skipIf(SUNOS,
                     "worthless on SUNOS (uses a subprocess)")
    def test_swap_memory(self):
        self.execute(psutilcopy.swap_memory)

    @unittest.skipIf(POSIX and SKIP_PYTHON_IMPL,
                     "worthless on POSIX (pure python)")
    def test_pid_exists(self):
        self.execute(psutilcopy.pid_exists, os.getpid())
    ...
    def test_disk_usage(self):
        self.execute(psutilcopy.disk_usage, '.')

    def test_disk_partitions(self):
        self.execute(psutilcopy.disk_partitions)

    @unittest.skipIf(LINUX and not os.path.exists('/proc/diskstats'),
                     '/proc/diskstats not available on this Linux version')
    @skip_if_linux()
    def test_disk_io_counters(self):
        self.execute(psutilcopy.disk_io_counters, nowrap=False)

    # --- proc

    @skip_if_linux()
    def test_pids(self):
        self.execute(psutilcopy.pids)

    # --- net

    @skip_if_linux()
    def test_net_io_counters(self):
        self.execute(psutilcopy.net_io_counters, nowrap=False)
    ...
    def test_net_connections(self):
        with create_sockets():
            self.execute(psutilcopy.net_connections)

    def test_net_if_addrs(self):
        # Note: verified that on Windows this was a false positive.
        self.execute(psutilcopy.net_if_addrs,
                     tolerance_=80 * 1024 if WINDOWS else None)

    @unittest.skipIf(TRAVIS, "EPERM on travis")
    def test_net_if_stats(self):
        self.execute(psutilcopy.net_if_stats)
    ...
    def test_sensors_battery(self):
        self.execute(psutilcopy.sensors_battery)

    @skip_if_linux()
    @unittest.skipIf(not HAS_SENSORS_TEMPERATURES, "not supported")
    def test_sensors_temperatures(self):
        self.execute(psutilcopy.sensors_temperatures)

    @skip_if_linux()
    @unittest.skipIf(not HAS_SENSORS_FANS, "not supported")
    def test_sensors_fans(self):
        self.execute(psutilcopy.sensors_fans)

    # --- others

    @skip_if_linux()
    def test_boot_time(self):
        self.execute(psutilcopy.boot_time)

    # XXX - on Windows this produces a false positive
    @unittest.skipIf(WINDOWS, "XXX produces a false positive on Windows")
    def test_users(self):
        self.execute(psutilcopy.users)
