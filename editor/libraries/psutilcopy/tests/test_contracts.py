...
from psutilcopy.tests import bind_unix_socket
from psutilcopy.tests import check_connection_ntuple
from psutilcopy.tests import get_kernel_version
from psutilcopy.tests import HAS_CONNECTIONS_UNIX
...
from psutilcopy.tests import is_namedtuple
...
from psutilcopy.tests import unix_socket_path
from psutilcopy.tests import VALID_PROC_STATUSES
from psutilcopy.tests import warn
import psutilcopy
...
class TestAvailability(unittest.TestCase):
    ...
    def test_cpu_affinity(self):
        ...
        self.assertEqual(hasattr(psutilcopy.Process, "cpu_affinity"), hasit)

    def test_win_service(self):
        self.assertEqual(hasattr(psutilcopy, "win_service_iter"), WINDOWS)
        self.assertEqual(hasattr(psutilcopy, "win_service_get"), WINDOWS)

    def test_PROCFS_PATH(self):
        self.assertEqual(hasattr(psutilcopy, "PROCFS_PATH"),
                         LINUX or SUNOS or AIX)

    def test_win_priority(self):
        ae = self.assertEqual
        ae(hasattr(psutilcopy, "ABOVE_NORMAL_PRIORITY_CLASS"), WINDOWS)
        ae(hasattr(psutilcopy, "BELOW_NORMAL_PRIORITY_CLASS"), WINDOWS)
        ae(hasattr(psutilcopy, "HIGH_PRIORITY_CLASS"), WINDOWS)
        ae(hasattr(psutilcopy, "IDLE_PRIORITY_CLASS"), WINDOWS)
        ae(hasattr(psutilcopy, "NORMAL_PRIORITY_CLASS"), WINDOWS)
        ae(hasattr(psutilcopy, "REALTIME_PRIORITY_CLASS"), WINDOWS)

    def test_linux_ioprio(self):
        ae = self.assertEqual
        ae(hasattr(psutilcopy, "IOPRIO_CLASS_NONE"), LINUX)
        ae(hasattr(psutilcopy, "IOPRIO_CLASS_RT"), LINUX)
        ae(hasattr(psutilcopy, "IOPRIO_CLASS_BE"), LINUX)
        ae(hasattr(psutilcopy, "IOPRIO_CLASS_IDLE"), LINUX)

    def test_linux_rlimit(self):
        ae = self.assertEqual
        hasit = LINUX and get_kernel_version() >= (2, 6, 36)
        ae(hasattr(psutilcopy.Process, "rlimit"), hasit)
        ae(hasattr(psutilcopy, "RLIM_INFINITY"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_AS"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_CORE"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_CPU"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_DATA"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_FSIZE"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_LOCKS"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_MEMLOCK"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_NOFILE"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_NPROC"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_RSS"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_STACK"), hasit)

        hasit = LINUX and get_kernel_version() >= (3, 0)
        ae(hasattr(psutilcopy, "RLIMIT_MSGQUEUE"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_NICE"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_RTPRIO"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_RTTIME"), hasit)
        ae(hasattr(psutilcopy, "RLIMIT_SIGPENDING"), hasit)

    def test_cpu_freq(self):
        linux = (LINUX and
                 (os.path.exists("/sys/devices/system/cpu/cpufreq") or
                  os.path.exists("/sys/devices/system/cpu/cpu0/cpufreq")))
        self.assertEqual(hasattr(psutilcopy, "cpu_freq"), linux or OSX or WINDOWS)

    def test_sensors_temperatures(self):
        self.assertEqual(hasattr(psutilcopy, "sensors_temperatures"), LINUX)

    def test_sensors_fans(self):
        self.assertEqual(hasattr(psutilcopy, "sensors_fans"), LINUX)

    def test_battery(self):
        self.assertEqual(hasattr(psutilcopy, "sensors_battery"),
                         LINUX or WINDOWS or FREEBSD or OSX)

    def test_proc_environ(self):
        self.assertEqual(hasattr(psutilcopy.Process, "environ"),
                         LINUX or OSX or WINDOWS)

    def test_proc_uids(self):
        self.assertEqual(hasattr(psutilcopy.Process, "uids"), POSIX)

    def test_proc_gids(self):
        self.assertEqual(hasattr(psutilcopy.Process, "uids"), POSIX)

    def test_proc_terminal(self):
        self.assertEqual(hasattr(psutilcopy.Process, "terminal"), POSIX)

    def test_proc_ionice(self):
        self.assertEqual(hasattr(psutilcopy.Process, "ionice"), LINUX or WINDOWS)

    def test_proc_rlimit(self):
        self.assertEqual(hasattr(psutilcopy.Process, "rlimit"), LINUX)

    def test_proc_io_counters(self):
        hasit = hasattr(psutilcopy.Process, "io_counters")
        self.assertEqual(hasit, False if OSX or SUNOS else True)

    def test_proc_num_fds(self):
        self.assertEqual(hasattr(psutilcopy.Process, "num_fds"), POSIX)

    def test_proc_num_handles(self):
        self.assertEqual(hasattr(psutilcopy.Process, "num_handles"), WINDOWS)

    def test_proc_cpu_affinity(self):
        self.assertEqual(hasattr(psutilcopy.Process, "cpu_affinity"),
                         LINUX or WINDOWS or FREEBSD)

    def test_proc_cpu_num(self):
        self.assertEqual(hasattr(psutilcopy.Process, "cpu_num"),
                         LINUX or FREEBSD or SUNOS)

    def test_proc_memory_maps(self):
        hasit = hasattr(psutilcopy.Process, "memory_maps")
...
class TestDeprecations(unittest.TestCase):
    ...
    def test_memory_info_ex(self):
        with warnings.catch_warnings(record=True) as ws:
            psutilcopy.Process().memory_info_ex()
        w = ws[0]
...
class TestSystem(unittest.TestCase):
    ...
    def test_io_counters(self):
        # Duplicate of test_system.py. Keep it anyway.
        for k in psutilcopy.disk_io_counters(perdisk=True):
            self.assertIsInstance(k, str)

    def test_disk_partitions(self):
        # Duplicate of test_system.py. Keep it anyway.
        for disk in psutilcopy.disk_partitions():
    ...
    def test_net_connections(self):
        with unix_socket_path() as name:
            with closing(bind_unix_socket(name)):
                cons = psutilcopy.net_connections(kind='unix')
                assert cons
                for conn in cons:
    ...
    def test_net_if_addrs(self):
        # Duplicate of test_system.py. Keep it anyway.
        for ifname, addrs in psutilcopy.net_if_addrs().items():
    ...
    def test_net_if_stats(self):
        # Duplicate of test_system.py. Keep it anyway.
        for ifname, _ in psutilcopy.net_if_stats().items():
            self.assertIsInstance(ifname, str)

    def test_net_io_counters(self):
        # Duplicate of test_system.py. Keep it anyway.
        for ifname, _ in psutilcopy.net_io_counters(pernic=True).items():
            self.assertIsInstance(ifname, str)

    @unittest.skipIf(not HAS_SENSORS_FANS, "not supported")
    def test_sensors_fans(self):
        # Duplicate of test_system.py. Keep it anyway.
        for name, units in psutilcopy.sensors_fans().items():
    ...
    def test_sensors_temperatures(self):
        # Duplicate of test_system.py. Keep it anyway.
        for name, units in psutilcopy.sensors_temperatures().items():
            self.assertIsInstance(name, str)
            for unit in units:
                self.assertIsInstance(unit.label, str)
...
class TestFetchAllProcesses(unittest.TestCase):
    ...
    def test_fetch_all(self):
        ...
        attrs = []
        for name in dir(psutilcopy.Process):
            ...
            with p.oneshot():
                for name in attrs:
                    ...
                    try:
                        ...
                        warn(msg)
                    except (psutilcopy.NoSuchProcess, psutilcopy.AccessDenied) as err:
    ...
    def create_time(self, ret, proc):
        ...
        try:
            ...
            # XXX
            if OPENBSD and proc.status() == psutilcopy.STATUS_ZOMBIE:
                pass
            else:
                raise
    ...
    def cpu_num(self, ret, proc):
        ...
        self.assertIn(ret, list(range(psutilcopy.cpu_count())))
        ...
        if ret:     # 'ret' can be None or empty
            ...
            try:
                ...
                if WINDOWS and err.errno in \
                        psutilcopy._psplatform.ACCESS_DENIED_SET:
                    pass
    ...
    def cpu_affinity(self, ret, proc):
        self.assertIsInstance(ret, list)
        assert ret != [], ret
        cpus = range(psutilcopy.cpu_count())
        for n in ret:
    ...
    def nice(self, ret, proc):
        ...
        if POSIX:
            ...
            priorities = [getattr(psutilcopy, x) for x in dir(psutilcopy)
