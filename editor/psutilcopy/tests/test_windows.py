...
cext = psutilcopy._psplatform.cext
...
def wrap_exceptions(fun):
    def wrapper(self, *args, **kwargs):
        try:
            ...
            from psutilcopy._pswindows import ACCESS_DENIED_SET
            if err.errno in ACCESS_DENIED_SET:
                raise psutilcopy.AccessDenied(None, None)
            if err.errno == errno.ESRCH:
                raise psutilcopy.NoSuchProcess(None, None)
            raise
    return wrapper
...
class TestCpuAPIs(unittest.TestCase):
    ...
    def test_cpu_count_vs_NUMBER_OF_PROCESSORS(self):
        ...
        self.assertEqual(num_cpus, psutilcopy.cpu_count())

    def test_cpu_count_vs_GetSystemInfo(self):
        # Will likely fail on many-cores systems:
        # https://stackoverflow.com/questions/31209256
        sys_value = win32api.GetSystemInfo()[5]
        psutil_value = psutilcopy.cpu_count()
        self.assertEqual(sys_value, psutil_value)

    def test_cpu_count_logical_vs_wmi(self):
        w = wmi.WMI()
        proc = w.Win32_Processor()[0]
        self.assertEqual(psutilcopy.cpu_count(), proc.NumberOfLogicalProcessors)

    def test_cpu_count_phys_vs_wmi(self):
        w = wmi.WMI()
        proc = w.Win32_Processor()[0]
        self.assertEqual(psutilcopy.cpu_count(logical=False), proc.NumberOfCores)

    def test_cpu_count_vs_cpu_times(self):
        self.assertEqual(psutilcopy.cpu_count(),
                         len(psutilcopy.cpu_times(percpu=True)))

    def test_cpu_freq(self):
        w = wmi.WMI()
        proc = w.Win32_Processor()[0]
        self.assertEqual(proc.CurrentClockSpeed, psutilcopy.cpu_freq().current)
        self.assertEqual(proc.MaxClockSpeed, psutilcopy.cpu_freq().max)
...
class TestSystemAPIs(unittest.TestCase):
    ...
    def test_nic_names(self):
        out = sh('ipconfig /all')
        nics = psutilcopy.net_io_counters(pernic=True).keys()
        for nic in nics:
    ...
    def test_total_phymem(self):
        ...
        self.assertEqual(int(w.TotalPhysicalMemory),
                         psutilcopy.virtual_memory().total)
    ...
    def test_pids(self):
        ...
        psutil_pids = set(psutilcopy.pids())
        self.assertEqual(wmi_pids, psutil_pids)

    @retry_before_failing()
    def test_disks(self):
        ps_parts = psutilcopy.disk_partitions(all=True)
            ...
            for wmi_part in wmi_parts:
                if ps_part.device.replace('\\', '') == wmi_part.DeviceID:
                    if not ps_part.mountpoint:
                        ...
                        break
                    try:
                        usage = psutilcopy.disk_usage(ps_part.mountpoint)
                    except OSError as err:
    ...
    def test_disk_usage(self):
        for disk in psutilcopy.disk_partitions():
            ...
            psutil_value = psutilcopy.disk_usage(disk.mountpoint)
    ...
    def test_disk_partitions(self):
        ...
        psutil_value = [x.mountpoint for x in psutilcopy.disk_partitions(all=True)]
        self.assertEqual(sys_value, psutil_value)
    ...
    def test_boot_time(self):
        ...
        psutil_dt = datetime.datetime.fromtimestamp(psutilcopy.boot_time())
    ...
    def test_boot_time_fluctuation(self):
        ...
        with mock.patch('psutil._pswindows.cext.boot_time', return_value=5):
            self.assertEqual(psutilcopy.boot_time(), 5)
        ...
        with mock.patch('psutil._pswindows.cext.boot_time', return_value=333):
            self.assertEqual(psutilcopy.boot_time(), 333)
...
class TestSensorsBattery(unittest.TestCase):
    ...
    def test_has_battery(self):
        if win32api.GetPwrCapabilities()['SystemBatteriesPresent']:
            self.assertIsNotNone(psutilcopy.sensors_battery())
        else:
    ...
    def test_percent(self):
        ...
        battery_psutil = psutilcopy.sensors_battery()
    ...
    def test_emulate_power_connected(self):
        with mock.patch("psutil._pswindows.cext.sensors_battery",
            ...
            self.assertEqual(psutilcopy.sensors_battery().secsleft,
...
class TestProcess(unittest.TestCase):
    ...
    def test_issue_24(self):
        ...
        self.assertRaises(psutilcopy.AccessDenied, p.kill)

    def test_special_pid(self):
        p = psutilcopy.Process(4)
        ...
        for name in dir(psutilcopy.Process):
            if name.startswith('_') \
                ...
                try:
                    ...
                    call(p, name)
                    num2 = p.num_handles()
                except (psutilcopy.NoSuchProcess, psutilcopy.AccessDenied):
                    pass
                else:
                    if num2 > num1:
                        fail = \
    ...
    def test_ctrl_signals(self):
        p = psutilcopy.Process(get_test_subprocess().pid)
        p.send_signal(signal.CTRL_C_EVENT)
        p.send_signal(signal.CTRL_BREAK_EVENT)
        p.kill()
        p.wait()
        self.assertRaises(psutilcopy.NoSuchProcess,
                          p.send_signal, signal.CTRL_C_EVENT)
        self.assertRaises(psutilcopy.NoSuchProcess,
                          p.send_signal, signal.CTRL_BREAK_EVENT)
    ...
    def test_nice(self):
        ...
        psutil_value = psutilcopy.Process().nice()
    ...
    def test_memory_info(self):
        ...
        psutil_value = psutilcopy.Process(self.pid).memory_info()
        self.assertEqual(
    ...
    def test_cpu_affinity(self):
        ...
        psutil_value = psutilcopy.Process(self.pid).cpu_affinity()
        self.assertEqual(psutil_value, sys_value)
    ...
    def test_io_counters(self):
        ...
        psutil_value = psutilcopy.Process().io_counters()
        self.assertEqual(
    ...
    def test_num_handles(self):
        ...
        psutil_value = psutilcopy.Process().num_handles()
...
class TestDualProcessImplementation(unittest.TestCase):
    ...
    def test_name(self):
        name = psutilcopy.Process(self.pid).name()
        with mock.patch("psutil._psplatform.cext.proc_exe",
                        side_effect=psutilcopy.AccessDenied(os.getpid())) as fun:
            self.assertEqual(psutilcopy.Process(self.pid).name(), name)
            assert fun.called

    def test_memory_info(self):
        mem_1 = psutilcopy.Process(self.pid).memory_info()
        with mock.patch("psutil._psplatform.cext.proc_memory_info",
                        side_effect=OSError(errno.EPERM, "msg")) as fun:
            mem_2 = psutilcopy.Process(self.pid).memory_info()
    ...
    def test_create_time(self):
        ctime = psutilcopy.Process(self.pid).create_time()
        with mock.patch("psutil._psplatform.cext.proc_create_time",
                        side_effect=OSError(errno.EPERM, "msg")) as fun:
            self.assertEqual(psutilcopy.Process(self.pid).create_time(), ctime)
            assert fun.called

    def test_cpu_times(self):
        cpu_times_1 = psutilcopy.Process(self.pid).cpu_times()
        with mock.patch("psutil._psplatform.cext.proc_cpu_times",
                        side_effect=OSError(errno.EPERM, "msg")) as fun:
            cpu_times_2 = psutilcopy.Process(self.pid).cpu_times()
            assert fun.called
            self.assertAlmostEqual(
    ...
    def test_io_counters(self):
        io_counters_1 = psutilcopy.Process(self.pid).io_counters()
        with mock.patch("psutil._psplatform.cext.proc_io_counters",
                        side_effect=OSError(errno.EPERM, "msg")) as fun:
            io_counters_2 = psutilcopy.Process(self.pid).io_counters()
    ...
    def test_num_handles(self):
        num_handles = psutilcopy.Process(self.pid).num_handles()
        with mock.patch("psutil._psplatform.cext.proc_num_handles",
                        side_effect=OSError(errno.EPERM, "msg")) as fun:
            self.assertEqual(psutilcopy.Process(self.pid).num_handles(),
                             num_handles)
            assert fun.called
...
class TestServices(unittest.TestCase):
    ...
    def test_win_service_iter(self):
        ...
        for serv in psutilcopy.win_service_iter():
            data = serv.as_dict()
            ...
            # win_service_get
            s = psutilcopy.win_service_get(serv.name())
            # test __eq__
            self.assertEqual(serv, s)

    def test_win_service_get(self):
        name = next(psutilcopy.win_service_iter()).name()

        with self.assertRaises(psutilcopy.NoSuchProcess) as cm:
            psutilcopy.win_service_get(name + '???')
        self.assertEqual(cm.exception.name, name + '???')

        # test NoSuchProcess
        service = psutilcopy.win_service_get(name)
        exc = WindowsError(
            psutilcopy._psplatform.cext.ERROR_SERVICE_DOES_NOT_EXIST, "")
        with mock.patch("psutil._psplatform.cext.winservice_query_status",
                        side_effect=exc):
            self.assertRaises(psutilcopy.NoSuchProcess, service.status)
        with mock.patch("psutil._psplatform.cext.winservice_query_config",
                        side_effect=exc):
            self.assertRaises(psutilcopy.NoSuchProcess, service.username)

        # test AccessDenied
        exc = WindowsError(
            psutilcopy._psplatform.cext.ERROR_ACCESS_DENIED, "")
        with mock.patch("psutil._psplatform.cext.winservice_query_status",
                        side_effect=exc):
            self.assertRaises(psutilcopy.AccessDenied, service.status)
        with mock.patch("psutil._psplatform.cext.winservice_query_config",
                        side_effect=exc):
            self.assertRaises(psutilcopy.AccessDenied, service.username)
