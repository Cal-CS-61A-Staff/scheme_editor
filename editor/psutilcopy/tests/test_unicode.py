...
from psutilcopy.tests import safe_rmpath as _safe_rmpath
...
class _BaseFSAPIsTests(object):
    ...
    def test_proc_name(self):
        ...
        if WINDOWS:
            ...
            with mock.patch("psutil._psplatform.cext.proc_exe",
                            side_effect=psutilcopy.AccessDenied(os.getpid())) as m:
                name = psutilcopy.Process(subp.pid).name()
                assert m.called
        else:
            name = psutilcopy.Process(subp.pid).name()
    ...
    def test_proc_connections(self):
        ...
        with unix_socket_path(suffix=suffix) as name:
            ...
            with closing(sock):
                conn = psutilcopy.Process().connections('unix')[0]
...
class TestWinProcessName(unittest.TestCase):
    ...
    def test_name_type(self):
        ...
        with mock.patch("psutil._psplatform.cext.proc_exe",
                        side_effect=psutilcopy.AccessDenied(os.getpid())) as m:
            self.assertIsInstance(psutilcopy.Process().name(), str)
            assert m.called
