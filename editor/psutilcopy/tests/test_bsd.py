...
class BSDSpecificTestCase(unittest.TestCase):
    ...
    def test_cpu_count_logical(self):
        syst = sysctl("hw.ncpu")
        self.assertEqual(psutilcopy.cpu_count(logical=True), syst)

    @unittest.skipIf(not which('sysctl'), "sysctl cmd not available")
    def test_virtual_memory_total(self):
        num = sysctl('hw.physmem')
        self.assertEqual(num, psutilcopy.virtual_memory().total)
...
class FreeBSDSpecificTestCase(unittest.TestCase):
    ...
    def test_proc_memory_maps(self):
        ...
        maps = psutilcopy.Process(self.pid).memory_maps(grouped=False)
        lines = out.split('\n')[1:]
        while lines:
            line = lines.pop()
            fields = line.split()
    ...
    def test_proc_exe(self):
        ...
        self.assertEqual(psutilcopy.Process(self.pid).exe(),
                         out.split('\n')[1].split()[-1])

    def test_proc_cmdline(self):
        out = sh('procstat -c %s' % self.pid)
        self.assertEqual(' '.join(psutilcopy.Process(self.pid).cmdline()),
                         ' '.join(out.split('\n')[1].split()[2:]))
    ...
    def test_vmem_active(self):
        ...
        self.assertAlmostEqual(psutilcopy.virtual_memory().active, syst,
                               delta=MEMORY_TOLERANCE)

    @retry_before_failing()
    def test_vmem_inactive(self):
        syst = sysctl("vm.stats.vm.v_inactive_count") * PAGESIZE
        self.assertAlmostEqual(psutilcopy.virtual_memory().inactive, syst,
                               delta=MEMORY_TOLERANCE)

    @retry_before_failing()
    def test_vmem_wired(self):
        syst = sysctl("vm.stats.vm.v_wire_count") * PAGESIZE
        self.assertAlmostEqual(psutilcopy.virtual_memory().wired, syst,
                               delta=MEMORY_TOLERANCE)

    @retry_before_failing()
    def test_vmem_cached(self):
        syst = sysctl("vm.stats.vm.v_cache_count") * PAGESIZE
        self.assertAlmostEqual(psutilcopy.virtual_memory().cached, syst,
                               delta=MEMORY_TOLERANCE)

    @retry_before_failing()
    def test_vmem_free(self):
        syst = sysctl("vm.stats.vm.v_free_count") * PAGESIZE
        self.assertAlmostEqual(psutilcopy.virtual_memory().free, syst,
                               delta=MEMORY_TOLERANCE)

    @retry_before_failing()
    def test_vmem_buffers(self):
        syst = sysctl("vfs.bufspace")
        self.assertAlmostEqual(psutilcopy.virtual_memory().buffers, syst,
                               delta=MEMORY_TOLERANCE)
    ...
    def test_muse_vmem_total(self):
        num = muse('Total')
        self.assertEqual(psutilcopy.virtual_memory().total, num)

    @unittest.skipIf(not MUSE_AVAILABLE, "muse not installed")
    @retry_before_failing()
    def test_muse_vmem_active(self):
        num = muse('Active')
        self.assertAlmostEqual(psutilcopy.virtual_memory().active, num,
                               delta=MEMORY_TOLERANCE)
    ...
    def test_muse_vmem_inactive(self):
        num = muse('Inactive')
        self.assertAlmostEqual(psutilcopy.virtual_memory().inactive, num,
                               delta=MEMORY_TOLERANCE)
    ...
    def test_muse_vmem_wired(self):
        num = muse('Wired')
        self.assertAlmostEqual(psutilcopy.virtual_memory().wired, num,
                               delta=MEMORY_TOLERANCE)
    ...
    def test_muse_vmem_cached(self):
        num = muse('Cache')
        self.assertAlmostEqual(psutilcopy.virtual_memory().cached, num,
                               delta=MEMORY_TOLERANCE)
    ...
    def test_muse_vmem_free(self):
        num = muse('Free')
        self.assertAlmostEqual(psutilcopy.virtual_memory().free, num,
                               delta=MEMORY_TOLERANCE)
    ...
    def test_muse_vmem_buffers(self):
        num = muse('Buffer')
        self.assertAlmostEqual(psutilcopy.virtual_memory().buffers, num,
                               delta=MEMORY_TOLERANCE)

    def test_cpu_stats_ctx_switches(self):
        self.assertAlmostEqual(psutilcopy.cpu_stats().ctx_switches,
                               sysctl('vm.stats.sys.v_swtch'), delta=1000)

    def test_cpu_stats_interrupts(self):
        self.assertAlmostEqual(psutilcopy.cpu_stats().interrupts,
                               sysctl('vm.stats.sys.v_intr'), delta=1000)

    def test_cpu_stats_soft_interrupts(self):
        self.assertAlmostEqual(psutilcopy.cpu_stats().soft_interrupts,
                               sysctl('vm.stats.sys.v_soft'), delta=1000)

    def test_cpu_stats_syscalls(self):
        self.assertAlmostEqual(psutilcopy.cpu_stats().syscalls,
    ...
    def test_swapmem_free(self):
        ...
        self.assertAlmostEqual(
            psutilcopy.swap_memory().free, free, delta=MEMORY_TOLERANCE)

    def test_swapmem_used(self):
        total, used, free = self.parse_swapinfo()
        self.assertAlmostEqual(
            psutilcopy.swap_memory().used, used, delta=MEMORY_TOLERANCE)

    def test_swapmem_total(self):
        total, used, free = self.parse_swapinfo()
        self.assertAlmostEqual(
            psutilcopy.swap_memory().total, total, delta=MEMORY_TOLERANCE)
    ...
    def test_boot_time(self):
        ...
        s = s[:s.find(',')]
        btime = int(s)
        self.assertEqual(btime, psutilcopy.boot_time())
    ...
    def test_sensors_battery(self):
        ...
        fields = dict([(x.split('\t')[0], x.split('\t')[-1])
                       for x in out.split("\n")])
        metrics = psutilcopy.sensors_battery()
        percent = int(fields['Remaining capacity:'].replace('%', ''))
        remaining_time = fields['Remaining time:']
        self.assertEqual(metrics.percent, percent)
        if remaining_time == 'unknown':
            self.assertEqual(metrics.secsleft, psutilcopy.POWER_TIME_UNLIMITED)
        else:
            self.assertEqual(secs2hours(metrics.secsleft), remaining_time)

    @unittest.skipIf(not HAS_BATTERY, "no battery")
    def test_sensors_battery_against_sysctl(self):
        self.assertEqual(psutilcopy.sensors_battery().percent,
                         sysctl("hw.acpi.battery.life"))
        self.assertEqual(psutilcopy.sensors_battery().power_plugged,
                         sysctl("hw.acpi.acline") == 1)
        secsleft = psutilcopy.sensors_battery().secsleft
        if secsleft < 0:
...
class OpenBSDSpecificTestCase(unittest.TestCase):
    ...
    def test_boot_time(self):
        ...
        psutil_bt = datetime.datetime.fromtimestamp(psutilcopy.boot_time())
...
class NetBSDSpecificTestCase(unittest.TestCase):
    ...
    def test_vmem_total(self):
        self.assertEqual(
            psutilcopy.virtual_memory().total, self.parse_meminfo("MemTotal:"))

    def test_vmem_free(self):
        self.assertAlmostEqual(
            psutilcopy.virtual_memory().free, self.parse_meminfo("MemFree:"),
            delta=MEMORY_TOLERANCE)

    def test_vmem_buffers(self):
        self.assertAlmostEqual(
            psutilcopy.virtual_memory().buffers, self.parse_meminfo("Buffers:"),
            delta=MEMORY_TOLERANCE)

    def test_vmem_shared(self):
        self.assertAlmostEqual(
            psutilcopy.virtual_memory().shared, self.parse_meminfo("MemShared:"),
            delta=MEMORY_TOLERANCE)

    def test_swapmem_total(self):
        self.assertAlmostEqual(
            psutilcopy.swap_memory().total, self.parse_meminfo("SwapTotal:"),
            delta=MEMORY_TOLERANCE)

    def test_swapmem_free(self):
        self.assertAlmostEqual(
            psutilcopy.swap_memory().free, self.parse_meminfo("SwapFree:"),
            delta=MEMORY_TOLERANCE)

    def test_swapmem_used(self):
        smem = psutilcopy.swap_memory()
    ...
    def test_cpu_stats_interrupts(self):
        ...
        self.assertAlmostEqual(
            psutilcopy.cpu_stats().interrupts, interrupts, delta=1000)
    ...
    def test_cpu_stats_ctx_switches(self):
        with open('/proc/stat', 'rb') as f:
            for line in f:
                ...
                raise ValueError("couldn't find line")
        self.assertAlmostEqual(
            psutilcopy.cpu_stats().ctx_switches, ctx_switches, delta=1000)
