...
from psutilcopy._common import memoize
from psutilcopy._common import memoize_when_activated
from psutilcopy._common import supports_ipv6
from psutilcopy._common import wrap_numbers
from psutilcopy._compat import PY3
from psutilcopy.tests import APPVEYOR
from psutilcopy.tests import bind_socket
from psutilcopy.tests import bind_unix_socket
from psutilcopy.tests import call_until
from psutilcopy.tests import chdir
from psutilcopy.tests import create_proc_children_pair
from psutilcopy.tests import create_sockets
from psutilcopy.tests import create_zombie_proc
from psutilcopy.tests import DEVNULL
from psutilcopy.tests import get_free_port
...
from psutilcopy.tests import HAS_MEMORY_FULL_INFO
...
from psutilcopy.tests import import_module_by_path
...
from psutilcopy.tests import retry
from psutilcopy.tests import ROOT_DIR
from psutilcopy.tests import run_test_module_by_name
from psutilcopy.tests import safe_mkdir
from psutilcopy.tests import safe_rmpath
from psutilcopy.tests import SCRIPTS_DIR
from psutilcopy.tests import sh
from psutilcopy.tests import tcp_socketpair
from psutilcopy.tests import TESTFN
from psutilcopy.tests import TOX
from psutilcopy.tests import TRAVIS
from psutilcopy.tests import unittest
from psutilcopy.tests import unix_socket_path
from psutilcopy.tests import unix_socketpair
from psutilcopy.tests import wait_for_file
from psutilcopy.tests import wait_for_pid
import psutilcopy
import psutilcopy.tests
...
class TestMisc(unittest.TestCase):
    ...
    def test_process__repr__(self, func=repr):
        ...
        with mock.patch.object(psutilcopy.Process, "name",
                               side_effect=psutilcopy.ZombieProcess(os.getpid())):
            p = psutilcopy.Process()
            r = func(p)
            self.assertIn("pid=%s" % p.pid, r)
    ...
    def test_no_such_process__repr__(self, func=repr):
        self.assertEqual(
            repr(psutilcopy.NoSuchProcess(321)),
            "psutil.NoSuchProcess process no longer exists (pid=321)")
        self.assertEqual(
            repr(psutilcopy.NoSuchProcess(321, name='foo')),
            "psutil.NoSuchProcess process no longer exists (pid=321, "
            "name='foo')")
        self.assertEqual(
            repr(psutilcopy.NoSuchProcess(321, msg='foo')),
            "psutil.NoSuchProcess foo")

    def test_zombie_process__repr__(self, func=repr):
        self.assertEqual(
            repr(psutilcopy.ZombieProcess(321)),
            "psutil.ZombieProcess process still exists but it's a zombie "
            "(pid=321)")
        self.assertEqual(
            repr(psutilcopy.ZombieProcess(321, name='foo')),
            "psutil.ZombieProcess process still exists but it's a zombie "
            "(pid=321, name='foo')")
        self.assertEqual(
            repr(psutilcopy.ZombieProcess(321, name='foo', ppid=1)),
            "psutil.ZombieProcess process still exists but it's a zombie "
            "(pid=321, name='foo', ppid=1)")
        self.assertEqual(
            repr(psutilcopy.ZombieProcess(321, msg='foo')),
            "psutil.ZombieProcess foo")

    def test_access_denied__repr__(self, func=repr):
        self.assertEqual(
            repr(psutilcopy.AccessDenied(321)),
            "psutil.AccessDenied (pid=321)")
        self.assertEqual(
            repr(psutilcopy.AccessDenied(321, name='foo')),
            "psutil.AccessDenied (pid=321, name='foo')")
        self.assertEqual(
            repr(psutilcopy.AccessDenied(321, msg='foo')),
            "psutil.AccessDenied foo")

    def test_timeout_expired__repr__(self, func=repr):
        self.assertEqual(
            repr(psutilcopy.TimeoutExpired(321)),
            "psutil.TimeoutExpired timeout after 321 seconds")
        self.assertEqual(
            repr(psutilcopy.TimeoutExpired(321, pid=111)),
            "psutil.TimeoutExpired timeout after 321 seconds (pid=111)")
        self.assertEqual(
            repr(psutilcopy.TimeoutExpired(321, pid=111, name='foo')),
            "psutil.TimeoutExpired timeout after 321 seconds "
            "(pid=111, name='foo')")

    def test_process__eq__(self):
        p1 = psutilcopy.Process()
        p2 = psutilcopy.Process()
    ...
    def test_process__hash__(self):
        s = set([psutilcopy.Process(), psutilcopy.Process()])
        self.assertEqual(len(s), 1)

    def test__all__(self):
        dir_psutil = dir(psutilcopy)
        for name in dir_psutil:
            ...
            if not name.startswith('_'):
                try:
                    ...
                    if name not in psutilcopy.__all__:
                        fun = getattr(psutilcopy, name)
                        if fun is None:
                            continue
        ...
        for name in psutilcopy.__all__:
            self.assertIn(name, dir_psutil)

    def test_version(self):
        self.assertEqual('.'.join([str(x) for x in psutilcopy.version_info]),
                         psutilcopy.__version__)
    ...
    def test_parse_environ_block(self):
        from psutilcopy._common import parse_environ_block
    ...
    def test_isfile_strict(self):
        from psutilcopy._common import isfile_strict
    ...
    def test_serialization(self):
        ...
        check(psutilcopy.Process().as_dict())
        check(psutilcopy.virtual_memory())
        check(psutilcopy.swap_memory())
        check(psutilcopy.cpu_times())
        check(psutilcopy.cpu_times_percent(interval=0))
        check(psutilcopy.net_io_counters())
        if LINUX and not os.path.exists('/proc/diskstats'):
            pass
        else:
            if not APPVEYOR:
                check(psutilcopy.disk_io_counters())
        check(psutilcopy.disk_partitions())
        check(psutilcopy.disk_usage(os.getcwd()))
        check(psutilcopy.users())
    ...
    def test_setup_script(self):
        ...
        self.assertEqual(module.get_version(), psutilcopy.__version__)

    def test_ad_on_process_creation(self):
        # We are supposed to be able to instantiate Process also in case
        # of zombie processes or access denied.
        with mock.patch.object(psutilcopy.Process, 'create_time',
                               side_effect=psutilcopy.AccessDenied) as meth:
            psutilcopy.Process()
            assert meth.called
        with mock.patch.object(psutilcopy.Process, 'create_time',
                               side_effect=psutilcopy.ZombieProcess(1)) as meth:
            psutilcopy.Process()
            assert meth.called
        with mock.patch.object(psutilcopy.Process, 'create_time',
...
class TestScripts(unittest.TestCase):
    ...
    def test_pidof(self):
        output = self.assert_stdout('pidof.py', psutilcopy.Process().name())
...
class TestSyncTestUtils(unittest.TestCase):
    ...
    def test_wait_for_pid(self):
        ...
        nopid = max(psutilcopy.pids()) + 99999
        with mock.patch('psutil.tests.retry.__iter__', return_value=iter([0])):
            self.assertRaises(psutilcopy.NoSuchProcess, wait_for_pid, nopid)
...
class TestProcessUtils(unittest.TestCase):
    ...
    def test_reap_children(self):
        ...
        p = psutilcopy.Process(subp.pid)
        assert p.is_running()
        reap_children()
        assert not p.is_running()
        assert not psutilcopy.tests._pids_started
        assert not psutilcopy.tests._subprocesses_started
    ...
    def test_create_proc_children_pair(self):
        ...
        children = psutilcopy.Process().children(recursive=True)
