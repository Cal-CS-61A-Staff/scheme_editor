...
from psutilcopy._compat import basestring
from psutilcopy._compat import PY3
from psutilcopy._compat import u
from psutilcopy.tests import call_until
from psutilcopy.tests import HAS_BATTERY
from psutilcopy.tests import HAS_CPU_FREQ
from psutilcopy.tests import HAS_RLIMIT
from psutilcopy.tests import MEMORY_TOLERANCE
from psutilcopy.tests import mock
from psutilcopy.tests import PYPY
from psutilcopy.tests import pyrun
from psutilcopy.tests import reap_children
from psutilcopy.tests import reload_module
...
from psutilcopy.tests import skip_on_not_implemented
from psutilcopy.tests import TESTFN
from psutilcopy.tests import ThreadTask
from psutilcopy.tests import TRAVIS
from psutilcopy.tests import unittest
from psutilcopy.tests import which
...
class TestSystemVirtualMemory(unittest.TestCase):
    ...
    def test_total(self):
        ...
        psutil_value = psutilcopy.virtual_memory().total
    ...
    def test_used(self):
        ...
        psutil_value = psutilcopy.virtual_memory().used
    ...
    def test_free(self):
        ...
        psutil_value = psutilcopy.virtual_memory().free
        self.assertAlmostEqual(
    ...
    def test_buffers(self):
        ...
        psutil_value = psutilcopy.virtual_memory().buffers
        self.assertAlmostEqual(
            vmstat_value, psutil_value, delta=MEMORY_TOLERANCE)
    ...
    def test_active(self):
        ...
        psutil_value = psutilcopy.virtual_memory().active
        self.assertAlmostEqual(
            vmstat_value, psutil_value, delta=MEMORY_TOLERANCE)
    ...
    def test_inactive(self):
        ...
        psutil_value = psutilcopy.virtual_memory().inactive
        self.assertAlmostEqual(
            vmstat_value, psutil_value, delta=MEMORY_TOLERANCE)
    ...
    def test_shared(self):
        ...
        psutil_value = psutilcopy.virtual_memory().shared
        self.assertAlmostEqual(
            free_value, psutil_value, delta=MEMORY_TOLERANCE,
            msg='%s %s \n%s' % (free_value, psutil_value, free.output))
    ...
    def test_available(self):
        ...
        if 'available' not in lines[0]:
            ...
            psutil_value = psutilcopy.virtual_memory().available
            self.assertAlmostEqual(
                free_value, psutil_value, delta=MEMORY_TOLERANCE,
    ...
    def test_avail_old_percent(self):
        ...
        # is off by max 10%.
        from psutilcopy._pslinux import calculate_avail_vmem
        from psutilcopy._pslinux import open_binary
...
class TestSystemSwapMemory(unittest.TestCase):
    ...
    def test_total(self):
        ...
        psutil_value = psutilcopy.swap_memory().total
    ...
    def test_used(self):
        ...
        psutil_value = psutilcopy.swap_memory().used
        return self.assertAlmostEqual(
            free_value, psutil_value, delta=MEMORY_TOLERANCE)
    ...
    def test_free(self):
        ...
        psutil_value = psutilcopy.swap_memory().free
        return self.assertAlmostEqual(
            free_value, psutil_value, delta=MEMORY_TOLERANCE)
    ...
    def test_meminfo_against_sysinfo(self):
        ...
        with mock.patch('psutil._pslinux.cext.linux_sysinfo') as m:
            swap = psutilcopy.swap_memory()
        assert not m.called
        import psutilcopy._psutil_linux as cext
...
class TestSystemCPU(unittest.TestCase):
    ...
    def test_cpu_times(self):
        fields = psutilcopy.cpu_times()._fields
    ...
    def test_cpu_count_logical_w_sysdev_cpu_online(self):
        ...
        if "-" in str(value):
            ...
            self.assertEqual(psutilcopy.cpu_count(), value)
    ...
    def test_cpu_freq_no_scaling_cur_freq_file(self):
        ...
        with mock.patch(patch_point, side_effect=open_mock):
            with mock.patch('glob.glob', return_value=policies):
                self.assertRaises(NotImplementedError, psutilcopy.cpu_freq)
...
class TestSystemCPUStats(unittest.TestCase):
    ...
    def test_ctx_switches(self):
        ...
        psutil_value = psutilcopy.cpu_stats().ctx_switches
        self.assertAlmostEqual(vmstat_value, psutil_value, delta=500)

    @unittest.skipIf(TRAVIS, "fails on Travis")
    def test_interrupts(self):
        vmstat_value = vmstat("interrupts")
        psutil_value = psutilcopy.cpu_stats().interrupts
        self.assertAlmostEqual(vmstat_value, psutil_value, delta=500)
...
class TestSystemNetwork(unittest.TestCase):
    ...
    def test_net_if_addrs_ips(self):
        for name, addrs in psutilcopy.net_if_addrs().items():
            for addr in addrs:
                if addr.family == psutilcopy.AF_LINK:
    ...
    def test_net_io_counters(self):
        ...
        nio = psutilcopy.net_io_counters(pernic=True, nowrap=False)
    ...
    def test_net_connections_ipv6_unsupported(self, supports_ipv6, inet_ntop):
        ...
        try:
            ...
            pass
        psutilcopy.net_connections(kind='inet6')
    ...
    def test_net_connections_mocked(self):
        with mock_open_content(
            ...
            textwrap.dedent("""\
                ...
                """)) as m:
            psutilcopy.net_connections(kind='unix')
            assert m.called
...
class TestSystemDisks(unittest.TestCase):
    ...
    def test_disk_partitions_mocked(self):
        ...
        with open("/proc/filesystems", "r") as f:
            data = f.read()
        if 'zfs' in data:
            for part in psutilcopy.disk_partitions():
                if part.fstype == 'zfs':
                    break
            else:
            ...
            with mock.patch('psutil._pslinux.open',
                ...
                with mock.patch(
                    ...
                    ret = psutilcopy.disk_partitions()
                    assert m1.called
                    assert m2.called
                    assert ret
    ...
    def test_disk_io_counters_kernel_2_4_mocked(self):
        ...
        with mock_open_content(
            ...
            with mock_open_content(
                ...
                ret = psutilcopy.disk_io_counters(nowrap=False)
...
class TestMisc(unittest.TestCase):
    ...
    def test_boot_time(self):
        ...
        psutil_value = psutilcopy.boot_time()
    ...
    def test_no_procfs_on_import(self, tb):
        ...
        try:
            ...
            patch_point = 'builtins.open' if PY3 else '__builtin__.open'
            with mock.patch(patch_point, side_effect=open_mock):
                reload_module(psutilcopy)
                assert tb.called

                self.assertRaises(IOError, psutilcopy.cpu_times)
                self.assertRaises(IOError, psutilcopy.cpu_times, percpu=True)
                self.assertRaises(IOError, psutilcopy.cpu_percent)
                self.assertRaises(IOError, psutilcopy.cpu_percent, percpu=True)
                self.assertRaises(IOError, psutilcopy.cpu_times_percent)
                self.assertRaises(
                    IOError, psutilcopy.cpu_times_percent, percpu=True)

                psutilcopy.PROCFS_PATH = my_procfs

                self.assertEqual(psutilcopy.cpu_percent(), 0)
                self.assertEqual(sum(psutilcopy.cpu_times_percent()), 0)

                # since we don't know the number of CPUs at import time,
                # we awkwardly say there are none until the second call
                per_cpu_percent = psutilcopy.cpu_percent(percpu=True)
                self.assertEqual(sum(per_cpu_percent), 0)

                # ditto awkward length
                per_cpu_times_percent = psutilcopy.cpu_times_percent(percpu=True)
                ...
                self.assertNotEqual(psutilcopy.cpu_percent(), 0)
                self.assertNotEqual(
                    sum(psutilcopy.cpu_percent(percpu=True)), 0)
                self.assertNotEqual(sum(psutilcopy.cpu_times_percent()), 0)
                self.assertNotEqual(
                    sum(map(sum, psutilcopy.cpu_times_percent(percpu=True))), 0)
        finally:
            shutil.rmtree(my_procfs)
            reload_module(psutilcopy)

        self.assertEqual(psutilcopy.PROCFS_PATH, '/proc')
    ...
    def test_cpu_steal_decrease(self):
        ...
        with mock_open_content(
            ...
            psutilcopy.cpu_percent()
            assert m.called
            psutilcopy.cpu_percent(percpu=True)
            psutilcopy.cpu_times_percent()
        ...
        with mock_open_content(
            ...
            cpu_percent = psutilcopy.cpu_percent()
            assert m.called
            cpu_percent_percpu = psutilcopy.cpu_percent(percpu=True)
            cpu_times_percent = psutilcopy.cpu_times_percent()
            cpu_times_percent_percpu = psutilcopy.cpu_times_percent(percpu=True)
    ...
    def test_users_mocked(self):
        ...
        with mock.patch('psutil._pslinux.cext.users',
            ...
            self.assertEqual(psutilcopy.users()[0].host, 'localhost')
            assert m.called
        with mock.patch('psutil._pslinux.cext.users',
        ...
        with mock.patch('psutil._pslinux.cext.users',
                        return_value=[('giampaolo', 'pts/2', 'foo',
                                       1436573184.0, True, 2)]) as m:
            self.assertEqual(psutilcopy.users()[0].host, 'foo')
            assert m.called

    def test_procfs_path(self):
        tdir = tempfile.mkdtemp()
        try:
            psutilcopy.PROCFS_PATH = tdir
            self.assertRaises(IOError, psutilcopy.virtual_memory)
            self.assertRaises(IOError, psutilcopy.cpu_times)
            self.assertRaises(IOError, psutilcopy.cpu_times, percpu=True)
            self.assertRaises(IOError, psutilcopy.boot_time)
            # self.assertRaises(IOError, psutil.pids)
            self.assertRaises(IOError, psutilcopy.net_connections)
            self.assertRaises(IOError, psutilcopy.net_io_counters)
            self.assertRaises(IOError, psutilcopy.net_if_stats)
            self.assertRaises(IOError, psutilcopy.disk_io_counters)
            self.assertRaises(IOError, psutilcopy.disk_partitions)
            self.assertRaises(psutilcopy.NoSuchProcess, psutilcopy.Process)
        finally:
            psutilcopy.PROCFS_PATH = "/proc"
            os.rmdir(tdir)
    ...
    def test_issue_687(self):
        ...
        t.start()
        try:
            p = psutilcopy.Process()
            tid = p.threads()[1].id
            assert not psutilcopy.pid_exists(tid), tid
            pt = psutilcopy.Process(tid)
            pt.as_dict()
            self.assertNotIn(tid, psutilcopy.pids())
        finally:
            t.stop()
    ...
    def test_pid_exists_no_proc_status(self):
        ...
        with mock_open_content("/proc/%s/status", "") as m:
            assert psutilcopy.pid_exists(os.getpid())
            assert m.called
...
class TestSensorsBattery(unittest.TestCase):
    ...
    def test_percent(self):
        ...
        psutil_value = psutilcopy.sensors_battery().percent
    ...
    def test_power_plugged(self):
        ...
        self.assertEqual(psutilcopy.sensors_battery().power_plugged, plugged)
    ...
    def test_emulate_power_plugged(self):
        ...
        with mock.patch(patch_point, side_effect=open_mock) as m:
            self.assertEqual(psutilcopy.sensors_battery().power_plugged, True)
            self.assertEqual(
                psutilcopy.sensors_battery().secsleft, psutilcopy.POWER_TIME_UNLIMITED)
            assert m.called
    ...
    def test_emulate_power_not_plugged(self):
        ...
        patch_point = 'builtins.open' if PY3 else '__builtin__.open'
        with mock.patch(patch_point, side_effect=open_mock) as m:
            self.assertEqual(psutilcopy.sensors_battery().power_plugged, False)
            assert m.called
    ...
    def test_emulate_power_undetermined(self):
        ...
        patch_point = 'builtins.open' if PY3 else '__builtin__.open'
        with mock.patch(patch_point, side_effect=open_mock) as m:
            self.assertIsNone(psutilcopy.sensors_battery().power_plugged)
            assert m.called
    ...
    def test_emulate_no_base_files(self):
        ...
        with mock_open_exception(
            ...
            with mock_open_exception(
                    ...
                    IOError(errno.ENOENT, "")):
                self.assertIsNone(psutilcopy.sensors_battery())

    def test_emulate_energy_full_0(self):
        # Emulate a case where energy_full files returns 0.
        with mock_open_content(
                "/sys/class/power_supply/BAT0/energy_full", b"0") as m:
            self.assertEqual(psutilcopy.sensors_battery().percent, 0)
            assert m.called
        ...
        with mock_open_exception(
            ...
            with mock_open_exception(
                ...
                with mock_open_content(
                    ...
                    self.assertEqual(psutilcopy.sensors_battery().percent, 88)
...
class TestSensorsTemperatures(unittest.TestCase):
    ...
    def test_emulate_eio_error(self):
        ...
        with mock.patch(patch_point, side_effect=open_mock) as m:
            with warnings.catch_warnings(record=True) as ws:
                self.assertEqual(psutilcopy.sensors_temperatures(), {})
                assert m.called
    ...
    def test_emulate_data(self):
        ...
        with mock.patch(patch_point, side_effect=open_mock):
            with mock.patch('glob.glob',
                ...
                temp = psutilcopy.sensors_temperatures()['name'][0]
    ...
    def test_emulate_data(self):
        ...
        with mock.patch(patch_point, side_effect=open_mock):
            with mock.patch('glob.glob',
                ...
                fan = psutilcopy.sensors_fans()['name'][0]
...
class TestProcess(unittest.TestCase):
    ...
    def test_terminal_mocked(self):
        with mock.patch('psutil._pslinux._psposix.get_terminal_map',
                        return_value={}) as m:
            self.assertIsNone(psutilcopy._pslinux.Process(os.getpid()).terminal())
            assert m.called
    ...
    def test_readlink_path_deleted_mocked(self):
        with mock.patch('psutil._pslinux.os.readlink',
            ...
            self.assertEqual(psutilcopy.Process().exe(), "/home/foo")
            self.assertEqual(psutilcopy.Process().cwd(), "/home/foo")
    ...
    def test_threads_mocked(self):
        ...
        patch_point = 'builtins.open' if PY3 else '__builtin__.open'
        with mock.patch(patch_point, side_effect=open_mock) as m:
            ret = psutilcopy.Process().threads()
            assert m.called
        ...
        with mock.patch('psutil._pslinux.readlink',
            ...
            with mock.patch('psutil.Process.cmdline',
                ...
                with mock.patch('psutil._pslinux.os.path.lexists',
                                return_value=False):
                    self.assertRaises(
                        psutilcopy.ZombieProcess, psutilcopy.Process().exe)
    ...
    def test_rlimit_zombie(self):
        ...
        with mock.patch("psutil._pslinux.cext.linux_prlimit",
            ...
            p = psutilcopy.Process()
            p.name()
            with self.assertRaises(psutilcopy.ZombieProcess) as exc:
                p.rlimit(psutilcopy.RLIMIT_NOFILE)
            assert m.called
    ...
    def test_stat_file_parsing(self):
        from psutilcopy._pslinux import CLOCK_TICKS
        ...
        with mock_open_content('/proc/%s/stat' % os.getpid(), content):
            ...
            self.assertEqual(p.status(), psutilcopy.STATUS_ZOMBIE)
            self.assertEqual(p.ppid(), 1)
            self.assertEqual(
                p.create_time(), 6 / CLOCK_TICKS + psutilcopy.boot_time())
            cpu = p.cpu_times()
...
class TestProcessAgainstStatus(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.proc = psutilcopy.Process()

    def read_status_file(self, linestart):
        with psutilcopy._psplatform.open_text(
...
class TestUtils(unittest.TestCase):
    ...
    def test_open_text(self):
        with psutilcopy._psplatform.open_text(__file__) as f:
            self.assertEqual(f.mode, 'rt')

    def test_open_binary(self):
        with psutilcopy._psplatform.open_binary(__file__) as f:
            self.assertEqual(f.mode, 'rb')

    def test_readlink(self):
        with mock.patch("os.readlink", return_value="foo (deleted)") as m:
            self.assertEqual(psutilcopy._psplatform.readlink("bar"), "foo")
            assert m.called
    ...
    def test_cat(self):
        ...
        with open(fname, "wt") as f:
            f.write("foo ")
        self.assertEqual(psutilcopy._psplatform.cat(TESTFN, binary=False), "foo")
        self.assertEqual(psutilcopy._psplatform.cat(TESTFN, binary=True), b"foo")
        self.assertEqual(
            psutilcopy._psplatform.cat(TESTFN + '??', fallback="bar"), "bar")
