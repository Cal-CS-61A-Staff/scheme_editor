...
from psutilcopy.tests import AF_UNIX
...
class Base(object):
    ...
    def compare_procsys_connections(self, pid, proc_cons, kind='all'):
        ...
        """
        try:
            sys_cons = psutilcopy.net_connections(kind=kind)
...
class TestUnconnectedSockets(Base, unittest.TestCase):
    ...
    def test_tcp_v4(self):
        ...
        with closing(bind_socket(AF_INET, SOCK_STREAM, addr=addr)) as sock:
            ...
            self.assertEqual(conn.status, psutilcopy.CONN_LISTEN)
    ...
    def test_udp_v4(self):
        ...
        with closing(bind_socket(AF_INET, SOCK_DGRAM, addr=addr)) as sock:
            conn = self.check_socket(sock)
            assert not conn.raddr
            self.assertEqual(conn.status, psutilcopy.CONN_NONE)
...
class TestConnectedSocketPairs(Base, unittest.TestCase):
    ...
    def test_tcp(self):
        ...
        try:
            ...
            self.assertEqual(cons[0].status, psutilcopy.CONN_ESTABLISHED)
            self.assertEqual(cons[1].status, psutilcopy.CONN_ESTABLISHED)
            ...
            for conn in cons:
                ...
                if p.pid == tcp4_proc.pid:
                    check_conn(p, conn, AF_INET, SOCK_STREAM, tcp4_addr, (),
                               psutilcopy.CONN_LISTEN,
                               ("all", "inet", "inet4", "tcp", "tcp4"))
                # UDP v4
                elif p.pid == udp4_proc.pid:
                    check_conn(p, conn, AF_INET, SOCK_DGRAM, udp4_addr, (),
                               psutilcopy.CONN_NONE,
...
class TestSystemWideConnections(Base, unittest.TestCase):
    ...
    def test_it(self):
        ...
        with create_sockets():
            from psutilcopy._common import conn_tmap
            for kind, groups in conn_tmap.items():
                # XXX: SunOS does not retrieve UNIX sockets.
                if kind == 'unix' and not HAS_CONNECTIONS_UNIX:
                    continue
                families, types_ = groups
                cons = psutilcopy.net_connections(kind)
                self.assertEqual(len(cons), len(set(cons)))
                check(cons, families, types_)

            self.assertRaises(ValueError, psutilcopy.net_connections, kind='???')

    @skip_on_access_denied()
    def test_multi_socks(self):
        with create_sockets() as socks:
            cons = [x for x in psutilcopy.net_connections(kind='all')
                    if x.pid == os.getpid()]
            self.assertEqual(len(cons), len(socks))
    ...
    def test_multi_sockets_procs(self):
        ...
        syscons = [x for x in psutilcopy.net_connections(kind='all') if x.pid
                   in pids]
        for pid in pids:
...
class TestMisc(unittest.TestCase):
    ...
    def test_connection_constants(self):
        ints = []
        strs = []
        for name in dir(psutilcopy):
            if name.startswith('CONN_'):
                num = getattr(psutilcopy, name)
                str_ = str(num)
                ...
                strs.append(str_)
        if SUNOS:
            psutilcopy.CONN_IDLE
            psutilcopy.CONN_BOUND
        if WINDOWS:
            psutilcopy.CONN_DELETE_TCB
